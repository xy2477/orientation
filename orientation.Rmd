...
For those cases with more than one age records, I think the first age recorded for the 1st hospital visit that the subjects said yes to tinnitus should be used for analysis.
For those controls with more than one age records, I think the last age recorded for the last hospital visit should be used for analysis.
...
summary(logmod)
library(rms)
logmod2 <- lrm(high_mpg ~ weight, data = auto_log)
logmod2
```
...
The overall goodness of fit for this logistic model is R2=0.726, which means 72.6% of the variability in car's mpg category can be explained by car's weight.
